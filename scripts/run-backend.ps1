...
try {
    if ($Debug) {
        Write-Host ">>> Debug mode enabled (reload where supported)"
        Write-Host ">>> Note: mcp-server still runs without reload"
    }
    ...
    Start-Bg -Label "lint-auditor (:8001)" -Name "agent-lint-auditor" -ScriptBlock {
        ...
        if ($using:Debug) {
            uv run --package agent-lint-auditor uvicorn agent_lint_auditor.__main__:app --host 0.0.0.0 --port 8001 --reload
        }
        else {
    ...
    Start-Bg -Label "orchestrator (:8000)" -Name "orchestrator" -ScriptBlock {
        ...
        if ($using:Debug) {
            ...
            uv run --package orchestrator uvicorn orchestrator.main:app --host 0.0.0.0 --port 8000
        }
    }
